...
#define max_search_buffer_length 400
#define max_lookahead_buffer_length 15
...
//	vector<string> vec;
	...
	if(filein.is_open()) {
		...
		while(filein.good() or look_ahead_buffer.size()!=0) {
			...
			while(look_ahead_buffer.size()<max_lookahead_buffer_length and filein.good()) {
				filein.get(ch);
			...
			//if(look_ahead_buffer.size()!=0) {
				...
				if(s=="") {
					...
					if(search_buffer.size()>=max_search_buffer_length) {
					...
					encrypt.length=len;
					if(len==max_lookahead_buffer_length) {
						filein.get(ch);
						encrypt.symbol=ch;
					}
